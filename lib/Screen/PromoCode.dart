...
class PromoCode extends StatefulWidget {
  ...
  const PromoCode({Key? key, required this.from, this.updateParent}) : super(key: key);
        ...
        apiBaseHelper.postAPICall(getPromoCodeApi, parameter).then((getdata) {
          ...
          if (!error) {
            ...
            if ((offset) < total) {
              ...
              tempList = (promo as List).map((data) => Promo.fromJson(data)).toList();
  ...
  Future<void> validatePromo(String promo) async {
    ...
    if (_isNetworkAvail) {
      try {
        ...
        setState(() {});
        var parameter = {USER_ID: CUR_USERID, PROMOCODE: promo, FINAL_TOTAL: oriPrice.toString()};
            ...
            ? _isLoading
                ...
                : promoList.isEmpty
                    ? Padding(
                        ...
                        child: Center(child: Text(getTranslated(context, 'NO_PROMCO')!)))
                    : RefreshIndicator(
                        ...
                        child: Padding(
                          ...
                          child: ListView.builder(
                              ...
                              shrinkWrap: true,
                              itemCount: (offset < total) ? promoList.length + 1 : promoList.length,
                              itemBuilder: (context, index) {
                                return (index == promoList.length && isLoadingmore)
