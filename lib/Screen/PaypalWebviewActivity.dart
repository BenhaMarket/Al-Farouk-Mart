...
class PaypalWebview extends StatefulWidget {
  ...
  const PaypalWebview({Key? key, this.url, this.from, this.msg, this.amt, this.orderId})
      : super(key: key);
...
class StatePayPalWebview extends State<PaypalWebview> {
  ...
  Future<void> AddTransaction(
      String tranId, String orderID, String status, String? msg, bool redirect) async {
    try {
      var parameter = {
        USER_ID: CUR_USERID,
        ORDER_ID: orderID,
      ...
      apiBaseHelper.postAPICall(addTransactionApi, parameter).then((getdata) {
        ...
        if (!error) {
          if (redirect) {
            ...
            Navigator.pushAndRemoveUntil(
                context,
                CupertinoPageRoute(builder: (BuildContext context) => const OrderSuccess()),
                ModalRoute.withName('/home'));
          }
        } else {
