...
import 'package:alfarouk_mart/Screen/Dashboard.dart';
import 'package:alfarouk_mart/Screen/HomePage.dart';
import 'package:alfarouk_mart/Screen/MyOrder.dart';
...
class PushNotificationService {
  ...
  PushNotificationService({required this.context, required this.pageController});
  ...
  Future initialise() async {
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage message) {
      ...
      if (type == "ticket_status") {
        Navigator.push(context, CupertinoPageRoute(builder: (context) => const CustomerSupport()));
    ...
    messaging.getInitialMessage().then((RemoteMessage? message) async {
      ...
      bool back =
          await Provider.of<SettingProvider>(context, listen: false).getPrefrenceBool(ISFROMBACK);
      ...
      if (message != null && back) {
        ...
        if (type == "products") {
          ...
          // Navigator.push(context, (CupertinoPageRoute(builder: (context) => const AllCategory())));
        } else if (type == "wallet") {
          ...
          Navigator.push(context, (CupertinoPageRoute(builder: (context) => const Splash())));
        }
        Provider.of<SettingProvider>(context, listen: false).setPrefrenceBool(ISFROMBACK, false);
      }
    });
  ...
  void _registerToken(String? token) async {
    ...
    Response response = await post(updateFcmApi, body: parameter, headers: headers)
  ...
  Future<void> getProduct(String id, int index, int secPos, bool list) async {
    try {
      ...
      Response response = await post(getProductApi, headers: headers, body: parameter)
          .timeout(const Duration(seconds: timeOut));
      var getdata = json.decode(response.body);
  ...
  selectNotificationPayload(String? payload) async {
    if (payload != null) {
      ...
      Navigator.push(
        context,
        CupertinoPageRoute(builder: (context) => MyApp(sharedPreferences: prefs)),
      );
    }
  }
}
...
Future<void> generateImageNotication(
    String title, String msg, String image, String type, String id, String url) async {
